...
class SecurityRedisConfig {
    ...
    fun securityRedisConnectionFactory(properties: SecurityProperties): RedisConnectionFactory {
        ...
        val clusterNodes = redisConfig.cluster?.nodes.orEmpty()
        val sentinelNodes = redisConfig.sentinel?.nodes.orEmpty()
        when {
            // 集群模式
            clusterNodes.isNotEmpty() -> {
                val clusterConfig = config.useClusterServers()
                clusterConfig.addNodeAddress(*clusterNodes.map { "redis://$it" }.toTypedArray())
            ...
            // 哨兵模式
            sentinelNodes.isNotEmpty() -> {
                val sentinelConfig = config.useSentinelServers()
                sentinelConfig.masterName = redisConfig.sentinel?.master
                sentinelConfig.addSentinelAddress(*sentinelNodes.map { "redis://$it" }.toTypedArray())
