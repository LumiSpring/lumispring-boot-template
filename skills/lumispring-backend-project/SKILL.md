...
Before editing a project, inspect its POM and the available LumiSpring source or artifact repository. Use the coordinates found there rather than copying stale snippets. For the current `4.1.1.0` source tree, the parent, BOM, and Starter group is `com.lumispring.framework`.
...
Default to Java 21, framework `4.1.1.0`, a standalone single-module JAR, `framework-starter-web`, and `security.enabled: false`. Never invent database passwords, Redis passwords, API keys, or production endpoints.
